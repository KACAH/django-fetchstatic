...
class Command(BaseCommand):
    ...
    def download_file(self, url, dest):
        ...
        if _con_length:
            ...
            log.info("Downloading %s (%s bytes)", _base_filename, _file_size)
        else:
            _file_size = UNKNOWN_SIZE
            log.info("Downloading %s (size unknown)", _base_filename)
    ...
    def get_file(self, file_params, folder):
        ...
        _download_filename = \
            os.path.join(self.temp_folder, _base_filename + ".part")
        try:
    ...
    def copy_zip(self, zip_includes, folder, only_min):
        ...
        for (_path, _dst) in zip_includes:
            _src = os.path.join(self.temp_folder, _path)
    ...
    def get_zip(self, zip_params, folder):
        ...
        _url = zip_params["url"]
        _zipname = self.get_file((_url, "."), self.temp_folder)
